  ...
  const campo = (key, label, type, placeholder, icon) => (
    <div>
      ...
      <div className="relative">
        ...
        <input type={type === 'password' && verPass ? 'text' : type} value={form[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} placeholder={placeholder} className="input-field pl-10" required={key !== 'telefono'} autoComplete={type === 'password' ? 'new-password' : key} />
        {key === 'password' && (
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex flex-col items-center gap-2">
            ...
            <span className="font-display font-bold text-2xl text-gray-900">Maxi<span className="text-primary-600">Despensa</span></span>
          </Link>
          <h1 className="font-display text-2xl font-bold text-gray-900 mt-4">Crear cuenta</h1>
          <p className="text-gray-500 text-sm">Únete a miles de clientes satisfechos</p>
        </div>
        ...
        <div className="card p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Confirmar contraseña</label>
              <div className="relative">
                <FiLock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type={verPass ? 'text' : 'password'} value={form.confirmar} onChange={e => setForm(p => ({ ...p, confirmar: e.target.value }))} placeholder="Repite tu contraseña" className={`input-field pl-10 ${form.confirmar && form.confirmar !== form.password ? 'border-red-400 ring-1 ring-red-400' : ''}`} required />
              </div>
              {form.confirmar && form.confirmar !== form.password && <p className="text-xs text-red-500 mt-1">Las contraseñas no coinciden</p>}
            </div>
            <button type="submit" disabled={cargando} className="btn-primary w-full py-3 text-base flex items-center justify-center disabled:opacity-60">
