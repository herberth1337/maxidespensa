...
export default function PedidoConfirmado() {
  ...
  useEffect(() => {
    api.get(`/orders/${id}`).then(r => setPedido(r.data.pedido));
  }, [id]);

  const handleDescargar = () => {
    setDescargando(true);
    try {
      generarFacturaPDF(pedido, usuario);
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12 text-center">
      ...
      <p className="mb-6" style={{ color:'var(--text-muted)' }}>
        Gracias por tu compra. Te notificaremos cuando tu pedido este en camino.
      </p>
      ...
      <div className="card p-5 text-left mb-5">
        ...
        <div className="mt-4 p-3 rounded-xl text-sm" style={{ background:'var(--bg-subtle)', color:'var(--text-secondary)' }}>
          <p className="font-semibold mb-1" style={{ color:'var(--text-primary)' }}>Direccion de entrega</p>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-3 px-6 rounded-xl font-semibold text-base mb-4 transition-all disabled:opacity-60"
        style={{ background:'var(--bg-subtle)', color:'var(--text-primary)', border:'1.5px solid var(--border-default)' }}
        ...
        onMouseLeave={e => { e.currentTarget.style.background='var(--bg-subtle)'; e.currentTarget.style.borderColor='var(--border-default)'; e.currentTarget.style.color='var(--text-primary)'; }}
      >
        {descargando
